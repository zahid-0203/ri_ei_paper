...
\begin{document}
	...
	Zafar, A. (2026). Evaluating visual grounding in multimodal medical reasoning. International Conference on Learning Representations (ICLR) Workshop.}
	...
	Explainability methods, such as Grad-CAM \cite{??}, show that a model may rely mainly on background and irrelevant regions instead of focused objects \cite{r5, r6, r7}. 
	...
	Previous studies have shown that background information and spurious correlations can influence deep neural network decisions and may lead to shortcut learning \cite{r9} \zahid{add more relevent refs}. 
	...
	We use Grad-CAM \cite{??} to identify the image regions contributing to the model's prediction. 
	...
	It is observed that in Grad-CAM visualizations, red and yellow regions in images are activated most i.e., red and yellow have greater influence than other colors on models' predictions \zahid{any ref to this??}. 
	...
	For example, a custom CNN achieves 99.45\% classification accuracy on the EPLI dataset, yet its mean LMOS is only 0.1900, resulting in the lowest ETI score of 0.5922 among the evaluated models. \zahid{what is the consequence?? e.g., XX\% of predictions are based on background??}
	In contrast, the MobileNetV2 \cite{??} also achieves 99.45\% accuracy but obtains a substantially higher mean LMOS of 0.7765 and an ETI of 0.8855. \zahid{what is the consequence?? e.g., XX\% of predictions are based on background, must be less than custom CNN??}
	...
	Conventional evaluation primarily measures the classification accuracy of a model. 
	Let \(acc\in[0,1]\) denote the classification accuracy of model %\(f_k\)

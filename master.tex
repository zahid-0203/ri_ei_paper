...
\begin{document}
	...
	Convolutional Neural Networks (CNNs) have achieved remarkable performance in a wide range of domains such as medical imaging, agriculture, and astronomy, where predictions support important decision-making processes \cite{r1, r2, r3, r4}. 
	In such applications, classification accuracy is an essential performance measure, but accuracy alone does not necessarily indicate whether a model is reliable. 
	A CNN model is considered more reliable when its predictions are grounded in task-relevant visual features rather than spurious correlations \cite{r5}.
	...
	Therefore, beyond predictive accuracy, models must be quantitatively evaluated to determine whether their predictions depend on task-relevant regions rather than irrelevant background cues. % continuing introducing the problem 
	Despite excellent accuracy, a model may not provide reliable predictions. Explainability methods, such as Grad-CAM \cite{r6}, show that a model may rely mainly on  background and irrelevant regions instead of  focused objects \cite{ r7, r8}. 
	This raises questions about reliability and trustworthiness. 
	...
	Model behavior can shift due to real-world perturbations e.g., \textit{noise}, \textit{blur}, \textit{lighting shifts}, \textit{compression}, and \textit{occlusion}. 
	These distortions can alter both CNN accuracy and the visual regions driving predictions. 
	Consequently, even when a model maintains relatively high accuracy under perturbations, 
	its underlying visual explanations may still become inconsistent \cite{r5,r9}. 
	Conventional robustness evaluation methods are primarily based on accuracy change under perturbation. 
	They do not capture whether the model maintains consistent reliance on task-relevant regions under perturbed conditions. 
	These observations motivate an evaluation ramework that considers both predictive behavior and explanation behavior.
	...
	In particular, Grad-CAM generates class-discriminative heatmaps that highlight image regions contributing to a model's predictions \cite{r6, r10}. 
	We refer to the Grad-CAM heatmaps as explanations of the corresponding model's predictions. 
	%The faithfulness of such explanations is an important consideration, since explanation scores should meaningfully reflect the influence of the corresponding image regions on the model’s predictions \cite{r10}. 
	Such explanations can reveal whether a model achieves high classification accuracy by focusing substantially on background or other irrelevant regions rather than target objects. Previous studies have shown that background information and spurious correlations can influence deep neural network decisions and may lead to shortcut learning \cite{r11, r12, r13} %\zahid{add more relevent refs}. 
	...
	%Motivated by these observations, we propose metrices combining accuracy and object focus.
	Motivated by these observations, we propose metrics that integrate accuracy with object focus.
	%This is the motivation for evaluating a framework that combines accuracy and object focus.
	% above paragraph sounds okay
	
	% related work and gap - missing
	\zahid{The gap of existing studies}

    
    Existing works have developed diverse approaches for evaluating CNN reliability, robustness, and explanation quality described in 
	Section \ref{related_works}. Reliability has been examined through uncertainty estimation, calibration, confidence, failure detection, 
	risk--coverage analysis, and broader trustworthiness criteria. Robustness has been evaluated through performance degradation under common 
	corruptions, natural distribution shifts, and other perturbations, while some studies have also investigated the stability of explanations 
	under corrupted inputs. Similarly, explanation quality and object reliance have been studied using faithfulness-based evaluation, ground-truth 
	explanations, and spatial correspondence between explanations and relevant image regions. These studies demonstrate that reliability, robustness, 
	and explanation quality are multidimensional concepts and can be quantified from different perspectives. However, the existing evaluation 
	approaches generally emphasize one or a subset of these dimensions, and they do not provide a unified model-level evaluation that simultaneously 
	considers predictive accuracy, the spatial focus of decision-relevant visual evidence, and the stability of both under input perturbations. 
	In particular, a model may exhibit high predictive accuracy or relatively stable accuracy under perturbations while relying on visual evidence that
	 is poorly aligned with the target object, or its visual focus may change substantially even when its predictive performance remains relatively 
	 stable. This motivates a complementary evaluation framework that jointly examines predictive performance, object-focused explanation quality, and 
	 explanation-aware robustness.

	
		
	% proposed solution
	%To address this limitation (\zahid{we did not say the limitation yet}), 
	We propose two novel evaluation metrics: Explainable Trustworthy Index (ETI) and Explainable Robustness Index (ERI) using XAI methods. 
	Our objective is not to modify or improve CNN models, rather to provide quantitative measures for evaluating and comparing their reliability and robustness. 
	...
	\begin{figure}[htbp]
		\centering
		\includegraphics[width=.98\columnwidth]{images/figure1.pdf}
		\caption{Comparison between traditional accuracy and the proposed XAI-based metrics (ETI and ERI). Although both Xception and the custom CNN achieve identical $99.45\%$ validation accuracy, Grad-CAM heatmaps reveal that the custom CNN relies on background context. 
		The proposed ETI and ERI metrics successfully capture this flaw by producing lower scores for the custom model.}
		\label{fig:figure1}
	\end{figure}
	
	% solution specific to the problem statement
	%Figure \ref{fig:figure1} illustrates the distinctive characteristic of our XAI based metrics over the traditinal accuracy.  
	%Here, we show the predictions of two models - Xception and a Custom CNN model (see \ref{sec:detials of Xception and Custom CNN}) on the same image. 
	%Both of the models are trained from the same training set for a leaf classification task and have achieved high accuraices (both $99.45\%$) on the same validation dataset. 
	%We compute the ETI and ERI scores of the models and find that Xception model has higher ETI and ERI scores (0.88 and 0.93 respectively) than the custom CNN model (0.59 and 0.79 respectively). 
	%Inspecting the Grad-CAM heatmap reveals that infact the custom CNN model predicts based on the background rather than the leaf object. 
	%Selecting the custom CNN based on the accuracy would be a wrong choice in this scenario and the proposed metrics indicating that. 
	Figure~\ref{fig:figure1} illustrates the distinct advantages of our XAI-based metrics over traditional accuracy. 
	Here, we show the predictions of two models - Xception and a custom CNN (detailed in Section~\ref{sec:detials of Xception and Custom CNN}) - on the same sample test image. 
	Both models were trained on identical training data for a leaf classification task, with each achieving an accuracy of $99.45\%$ on the validation set. 
	However, evaluating the models using our proposed metrics reveals that Xception achieves significantly higher ETI and ERI scores ($0.88$ and $0.93$) compared to the custom CNN ($0.59$ and $0.79$). 
	Inspecting the Grad-CAM heatmaps confirms that the custom CNN relies heavily on background rather than the leaf object itself. 
	While standard accuracy fails to identify this flaw, our proposed metrics correctly indicate that selecting the custom CNN would lead to sub-optimal model deployment.
	
	% solution idea
	We introduce the Localization Mask Overlap Score (LMOS) to quantify object-focused explanation quality which is the proportion of the model's decision-relevant activation that overlaps with the annotated target-object region. 
	LMOS is used in both ETI and ERI calculation.  
	...
	It is observed that in Grad-CAM visualizations, red and yellow regions are activated most i.e., red and yellow have greater influence than other colors on models' predictions \cite{r14,r15}. 
	%So we consider them as decision-relevant regions. \zahid{any ref to this??}. 
	...
	% have not introduced purturbation yet -- skip this now
	%We evaluate nine perturbations, including \textit{Gaussian noise}, \textit{Gaussian blur}, \textit{brightness adjustment}, \textit{hue shift}, \textit{contrast adjustment}, \textit{gamma correction}, \textit{motion blur}, \textit{JPEG compression}, \textit{and random occlusion}. 
	...
	We comprehensively evaluate the proposed metrics using four real-world datasets comprising two leaf image datasets:  EPLI \cite{r16}, BDML\cite{r17} and two flower image datasets: FLRS \cite{r18}, FR \cite{r19}. 
	Eight CNN architectures are trained and evaluated on the datasets. 
	%We demonstrate the importance of combining accuracy with LMOS in ETI. 
	%For example, a custom CNN achieves 99.45\% classification accuracy on the EPLI dataset, yet its mean LMOS is only 0.1900, resulting in the lowest ETI score of 0.5922 among the evaluated models. Its LMOS is only 0.1900, indicating that only 19.00\% of its decision-relevant activation, on average, falls within the annotated target-object regions, while the remaining 81.00\% lies outside these regions. 
	%\zahid{what is the consequence??  e.g., XX\% of predictions are based on background??}
	We demonstrate that a custom CNN  model achieves 99.45\% classification accuracy on the EPLI dataset, yet its mean LMOS is only 0.1900, resulting in the lowest ETI score of 0.5922 among the evaluated models. 
	Its LMOS is only 0.1900, indicating that only 19.00\% of its decision-relevant activation, on average, falls within the annotated target-object regions, while the remaining 81.00\% lies outside these regions. 
	In contrast, the MobileNetV2 model achieves 99.45\% accuracy (same as the custom CNN model) but obtains a substantially higher mean LMOS of 0.7765, indicating that 77.65\% of its decision-relevant activation falls within the target-object regions, with only 22.35\% outside them and an ETI of 0.8855. 
	%\zahid{what is the consequence?? e.g., XX\% of predictions are based on background, must be less than custom CNN??}
	...
	The proposed ERI combines \emph{accuracy stability} and \emph{LMOS stability} to provide a joint measure of explainable robustness. 
	This distinction is important because predictive stability and explanation stability are not correlated. 
	For example, in our experiments we observe that the Custom CNN model under contrast perturbation maintains relatively high accuracy stability of 0.8494 but has substantially lower LMOS stability of 0.4545 on the EPLI dataset. 
	This indicates that, although its predictive performance remains comparatively stable, its object-focused explanation changes substantially relative to the clean-image condition. 
	Conversely, the ResNet50 model exhibits relatively low accuracy stability of 0.4599 but higher LMOS stability of 0.7830 under the same perturbation, indicating that its object-focused explanation remains comparatively more consistent despite a larger change in predictive performance. 
...
%
	...
	We propose Explainable Robustness Index (ERI) to address this problem. 
	Table \ref{tab:notation} describes the notations used in the article.
	...
	\section{Explainable Trustworthy Index and Explainable Robustness Index}
	...
	%\noindent
	\subsection{Localization Mask Overlap Score (LMOS).} 

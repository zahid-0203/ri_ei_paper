...
\begin{document}
	...
	Convolutional Neural Networks (CNNs) have achieved remarkable performance in a wide range of domains such as medical imaging, agriculture, and astronomy, where predictions support important decision-making processes  \cite{r1}\cite{r2}\cite{r3}\cite{r4}. 
	...
	A CNN model is considered reliable when its predictions are strictly grounded in task-relevant visual features rather than spurious correlations.
	Therefore, beyond predictive accuracy, models must be quantitatively evaluated to determine whether their predictions depend on task-relevant regions rather than irrelevant background cues. 
	% continuing introducing the problem
	Despite excellent accuracy, a model may not provide reliable predictions. 
	Explainability methods, such as Grad-CAM \cite{r5}, show that a model may rely mainly on background and irrelevant regions instead of focused objects \cite{r5, r6, r7}. 
	...
	Model behavior can shift due to real-world perturbations e.g., noise, blur, lighting shifts, compression, and occlusion. 
	These distortions can alter both CNN accuracy and the visual regions driving predictions. 
	Consequently, even when a model maintains relatively high accuracy under perturbations, its underlying visual explanations may still become inconsistent \cite{r4,r5,r9}.
	Conventional robustness evaluation methods are 
	primarily based on accuracy change under perturbation. 
	They do not capture whether the model maintains consistent reliance on task-relevant regions under perturbed conditions. These observations motivate an evaluation framework that considers both predictive behavior and explanation behavior.
	...
	In particular, Grad-CAM generates class-discriminative heatmaps that highlight image regions contributing to a model's prediction \cite{r5,r6,r7}. 
	Such explanations can reveal whether a model achieves high classification accuracy by focusing substantially on background or other irrelevant regions rather than target objects. 
	Previous studies have shown that background information and spurious correlations can influence deep neural network decisions and may lead to shortcut learning \cite{r9}. 
	...
	In essence, ETI and ERI address two different questions: ETI asks ``Whether a model is accurate and appropriately focused on the target object under normal conditions?'', whereas ERI asks ``Whether these properties remain stable when the input is perturbed?''
	...
	We use Grad-CAM \cite{r5} to identify the image regions contributing to the model's prediction. 
	...
	It is observed that in Grad-CAM visualizations, red and yellow regions are typically the most activated and therefore have the strongest influence on the model's predictions. 
	...
	To comprehensively evaluate the proposed metrics, we conduct experiments using four public real-world datasets comprising two leaf image datasets:  EPLI \cite{r10}, BDML\cite{r11} and two flower image datasets: FLRS \cite{r12}, FR \cite{r13}. 
	...
	We evaluate nine perturbations, including Gaussian noise, Gaussian blur, brightness adjustment, hue shift, contrast adjustment, gamma correction, motion blur, JPEG compression, and random occlusion. 
	...
	For example, a custom CNN achieves 99.45\% classification accuracy on the EPLI dataset, yet its mean LMOS is only 0.1900, resulting in the lowest ETI score of 0.5922 among the evaluated models. 
	In contrast, the MobileNetV2 architecture also achieves 99.45\% accuracy but obtains a substantially higher mean LMOS of 0.7765 and an ETI of 0.8855. 
	...
	For example, on the EPLI dataset, the Custom CNN under contrast perturbation maintains relatively high accuracy stability of 0.8494 but has substantially lower LMOS stability of 0.4545. 
	Conversely, ResNet50 under the same perturbation exhibits relatively low accuracy stability of 0.4599 but higher LMOS stability of 0.7830. 
	...
	Let  $\mathcal{D}=\{(x_i,y_i,M_i)\}_{i=1}^{N}$ be a dataset containing $N$ images, where $x_i\in \mathbb R^{n\times m}$ is the $i$-th image, $y_i$ is its ground-truth label, and $M_i$ is the corresponding ground-truth mask of the target object. 
	...
	We define a scoring function $\mathrm{LMOS}_i \in [0, 1]$ to measure the overlap ratio between the decision-relevant regions of $H_i$ and the ground-truth object mask $M_i$.
	...
	Firstly, we want to evaluate CNN models considering both their accuracy ($acc$) and object-focused explanation quality ($\mathrm{LMOS}$). 
	...
	Let $S_{\mathrm{acc}}$ and $S _ {\mathrm{LMOS}}$ represent the overall accuracy stability (the model's ability to perform under purturbation) and LMOS stability (the model's ability to use the object-focused are for making predictions) of a model. 
	...
	\begin{itemize}
		...
		%, including both normal and diseased leaves.
	...
	LMOS measures the consistency between model-generated explanations and ground-truth object regions. 
	We briefly describe the implementation details of these metrics below. 
	
	 %\noindent
	\subsubsection{Localization Mask Overlap Score (LMOS).} 
	LMOS is an instance-level measure that quantifies how much the model's decision-relevant activation overlaps with the true object region. Unlike accuracy, which is a dataset-level summary, LMOS evaluates the explanation quality for each image and can be averaged across the test set. 
	Let $H \in \mathbb{R}^{h \times w}$ denote the Grad-CAM heatmap generated from the last convolutional layer of a network. 
	$H$ is resized to the model's input image size. 
	...
	%\end{equation}
	where $\mathbb{I}(\cdot)$ is an indicator function and $\tau$ is the threshold. 
	We set $\tau=0.7$ to restrict LMOS calculations to the most highly activated red and yellow regions of the Grad-CAM map, which correspond to the pixels with the strongest influence on the prediction. 
	...
	\end{equation}
	where $M \in \{0,1\}^{h \times w}$ denotes the resized ground-truth binary segmentation mask, and $\hat{H}(i,j)$ is the thresholded activation map. 
	...
	We provide equal weighting to both components to avoid favoring either predictive performance or explanatory quality. 
	...
	We introduce the Accuracy Stability ($S_{\rm{acc}}$) score under a specific perturbation $p$ as:
	%begin{equation}
	...
	\textbf{Similarly, we define LMOS Stability to measure how closely a model's object reliance under perturbation remains consistent with its behavior on the clean image.}
...
%	\subsubsection{LMOS Stability Under Perturbations }
	...
	The details of the perturbations are given below:
	...
	We used TensorFlow/Keras on the Kaggle environment with two NVIDIA Tesla T4 GPUs for the training and evaluation of all CNN models in our experiments. 
	The training configuration follows the settings adopted in the original EgyPLI study \cite{r10}. 
	The custom CNN was trained from scratch, while other models were initialized with ImageNet-pretrained weights and their backbone layers were frozen.
	
	For the pretrained models, the original classification heads were removed and replaced with a common classification head consisting of Global Average Pooling, a dropout layer with dropout rate 0.3, a fully connected layer with 128 ReLU units, and a final softmax layer configured according to the number of classes in each dataset. 
	%The custom CNN has followed its original convolutional architecture. 
	The Adam optimiser was used with a learning rate of $5\times10^{-5}$ and categorical cross-entropy loss in all model optimizations. 
